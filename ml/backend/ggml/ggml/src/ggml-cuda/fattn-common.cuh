...
// WHT-TQ flash attention K dot products
#include "wht.cuh"
...
template<int D, int nthreads>
static __device__ __forceinline__ float vec_dot_fattn_vec_KQ_tq4_0_wht(
    const char * __restrict__ K_c, const void * __restrict__ Q_v, const int * __restrict__ Q_q8, const void * __restrict__ Q_ds_v) {
...
// WHT-TQ V dequantization for flash attention
template <typename T, int ne>
static __device__ __forceinline__ void dequantize_V_tq3_0_wht(const void * __restrict__ vx, void * __restrict__ dst, const int64_t i0) {
    const block_tq3_0 * x = (const block_tq3_0 *) vx;
...
#pragma unroll
        for (int l0 = 0; l0 < ne; l0 += 2) {
            ...
            const uint8_t s0 = (x[ib].signs[j0/8] >> (j0%8)) & 0x1;
            const uint8_t s1 = (x[ib].signs[j1/8] >> (j1%8)) & 0x1;
            const float v0 = d * WHT_CODEBOOK_4[a0] * (1.0f - 2.0f * s0);
            const float v1 = d * WHT_CODEBOOK_4[a1] * (1.0f - 2.0f * s1);
            ((half2 *) dst)[l0/2] = make_half2(v0, v1);
        }
    } else
#endif // FP16_AVAILABLE
...
#pragma unroll
        for (int l = 0; l < ne; ++l) {
            ...
            const uint8_t a = (x[ib].al[j/4] >> ((j%4)*2)) & 0x3;
            const uint8_t s = (x[ib].signs[j/8] >> (j%8)) & 0x1;
            ((float *) dst)[l] = d * WHT_CODEBOOK_4[a] * (1.0f - 2.0f * s);
        }
    } else {
        static_assert(std::is_same_v<T, void>, "unsupported type");
    }
...
template <typename T, int ne>
static __device__ __forceinline__ void dequantize_V_tq4_0_wht(const void * __restrict__ vx, void * __restrict__ dst, const int64_t i0) {
    const block_tq4_0 * x = (const block_tq4_0 *) vx;
...
#pragma unroll
        for (int l0 = 0; l0 < ne; l0 += 2) {
            ...
            const uint8_t s0 = (x[ib].signs[j0/8] >> (j0%8)) & 0x1;
            const uint8_t s1 = (x[ib].signs[j1/8] >> (j1%8)) & 0x1;
            const float v0 = d * WHT_CODEBOOK_8[idx0] * (1.0f - 2.0f * s0);
            const float v1 = d * WHT_CODEBOOK_8[idx1] * (1.0f - 2.0f * s1);
            ((half2 *) dst)[l0/2] = make_half2(v0, v1);
        }
    } else
#endif // FP16_AVAILABLE
...
#pragma unroll
        for (int l = 0; l < ne; ++l) {
            ...
            const uint8_t idx = lo | (hi << 2);
            const uint8_t s = (x[ib].signs[j/8] >> (j%8)) & 0x1;
            ((float *) dst)[l] = d * WHT_CODEBOOK_8[idx] * (1.0f - 2.0f * s);
        }
    } else {
        static_assert(std::is_same_v<T, void>, "unsupported type");
    }
...
static __device__ __forceinline__ float vec_dot_fattn_vec_KQ_turbo4_0(
    ...
    // turbo4 now uses simple d/al/ah/signs layout (same as TQ4_0)
...
#pragma unroll
        for (int k_KQ_1 = 0; k_KQ_1 < cpy_ne; k_KQ_1 += 2) {
            ...
            const int jbase = elem % QK_TURBO4;
            const float d_k = __half2float(K_turbo[ib].d);

            float v[4];
#pragma unroll
            for (int l = 0; l < 4; l++) {
                const int j = jbase + l;
                const uint8_t lo = (K_turbo[ib].al[j/4] >> ((j%4)*2)) & 0x3;
                const uint8_t hi = (K_turbo[ib].ah[j/8] >> (j%8)) & 0x1;
                const uint8_t idx = lo | (hi << 2);
                const uint8_t s = (K_turbo[ib].signs[j/8] >> (j%8)) & 0x1;
                v[l] = d_k * grid[idx] * (1.0f - 2.0f * s);
            }
...
static __device__ __forceinline__ void dequantize_V_turbo4_0(const void * __restrict__ vx, void * __restrict__ dst, const int64_t i0) {
    ...
    // turbo4 now uses simple d/al/ah/signs layout
    constexpr float grid[8] = {
        1.0f/16.0f, 3.0f/16.0f, 5.0f/16.0f, 7.0f/16.0f,
        9.0f/16.0f, 11.0f/16.0f, 13.0f/16.0f, 15.0f/16.0f
    };
...
#pragma unroll
        for (int l0 = 0; l0 < ne; l0 += 2) {
            ...
            const uint8_t hi1 = (x[ib].ah[j1/8] >> (j1%8)) & 0x1;
            const uint8_t s1  = (x[ib].signs[j1/8] >> (j1%8)) & 0x1;
            const float f0 = d * grid[lo0|(hi0<<2)] * (1.0f - 2.0f * s0);
            const float f1 = d * grid[lo1|(hi1<<2)] * (1.0f - 2.0f * s1);
            ((half2 *) dst)[l0/2] = make_half2(__float2half(f0), __float2half(f1));
        }
    } else
#endif // FP16_AVAILABLE
...
#pragma unroll
        for (int l = 0; l < ne; ++l) {
            ...
            const uint8_t hi = (x[ib].ah[j/8] >> (j%8)) & 0x1;
            const uint8_t s  = (x[ib].signs[j/8] >> (j%8)) & 0x1;
            ((float *) dst)[l] = d * grid[lo|(hi<<2)] * (1.0f - 2.0f * s);
        }
    } else {
        static_assert(std::is_same_v<T, void>, "unsupported type");
    }
...
constexpr __device__ vec_dot_KQ_t get_vec_dot_KQ() {
    ...
    } else if constexpr (type_K == GGML_TYPE_TQ3_0_WHT) {
        return vec_dot_fattn_vec_KQ_tq3_0_wht<D, nthreads>;
    } else if constexpr (type_K == GGML_TYPE_TQ4_0_WHT) {
        return vec_dot_fattn_vec_KQ_tq4_0_wht<D, nthreads>;
...
constexpr __device__ dequantize_V_t get_dequantize_V() {
    ...
    } else if constexpr (type_V == GGML_TYPE_TQ3_0_WHT) {
        return dequantize_V_tq3_0_wht<T, ne>;
    } else if constexpr (type_V == GGML_TYPE_TQ4_0_WHT) {
        return dequantize_V_tq4_0_wht<T, ne>;
